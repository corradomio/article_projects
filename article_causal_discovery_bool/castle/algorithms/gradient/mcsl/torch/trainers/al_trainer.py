...
class Trainer(object):
    ...
    def __init__(self, model, learning_rate, init_rho, rho_thresh, h_thresh,
        ...
        self._log = logging.getLogger("castle.algorithms.mscsl.Trainer")
        ...
        for i in range(1, max_iter + 1):
            ...
            while rho < self.rho_thresh:
                loss_new, h_new, w_logits_new = self.train_step(
                    x, iter_step, rho, alpha, self.temperature, i
                )
    ...
    def train_step(self, x, iter_step, rho, alpha, temperature, epoch) -> tuple:
        ...
        for _ in range(iter_step):
            ...
            if _ % LOG_FREQUENCY == 0:
                self._log.info(f'epoch={epoch}/{_}: loss={curr_loss.detach()}')
