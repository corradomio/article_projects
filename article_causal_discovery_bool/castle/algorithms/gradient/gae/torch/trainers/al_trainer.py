...
class ALTrainer(object):
    ...
    def __init__(self, n, d, model, lr, init_iter, alpha, beta, rho, rho_thresh,
        ...
        self._log = logging.getLogger("castle.algorithms.gae.ALTrainer")
    ...
    def train(self, x, epochs, update_freq):
        ...
        for epoch in range(1, epochs + 1):
            ...
            # if epoch%10 == 0:
            self._log.info(f"... epoch={epoch}, h={h}, alpha={alpha}")
        # end
        # self._log.info("done training")
        return w_est


    def train_step(self, x, update_freq, alpha, rho, epoch):
        ...
        for _ in range(update_freq):
            ...
            if _ % LOG_FREQUENCY == 0:
                self._log.info(f'epoch={epoch}/{_}: loss={loss.detach()}')
