...
class RL(BaseLearner):
    ...
    def __init__(self, encoder_type='TransformerEncoder', 
        ...
        if torch.cuda.is_available():
            # self._log.info('GPU is available.')
            pass
        else:
            # self._log.info('GPU is unavailable.')
    ...
    def _rl(self, X):
        ...
        # self._log.info('Python version is {}'.format(platform.python_version()))
        ...
        if self.lambda_flag_default:
            ...
            self._log.info('Original sl: {}, su: {}, strue: {}'.format(sl, su, strue))
            self._log.info('Transfomed sl: {}, su: {}, lambda2: {}, true: {}'.format(sl, su, lambda2,
        ...
        self._log.info('Finished creating training dataset and reward class')
        ...
        for i in tqdm(range(1, self.nb_epoch + 1)):
            ...
            if self.verbose:
                self._log.info('Start training for {}-th epoch'.format(i))
            ...
            # Test tensor shape
            if i == 1:
                self._log.info('Shape of actor.input: {}'.format(inputs.shape))
            ...
            if self.verbose:
                self._log.info('Finish calculating reward for current batch of graph')
            ...
            if self.verbose:
                self._log.info('Finish updating actor and critic network using reward calculated')
            ...
            # logging
            if i == 1 or i % 500 == 0:
                self._log.info('[iter {}] reward_batch: {:.4}, max_reward: {:.4}, max_reward_batch: {:.4}'.format(i,
            ...
            if i == 1 or i % lambda_iter_num == 0:
                ...
                self._log.info('[iter {}] lambda1 {:.4}, upper {:.4}, lambda2 {:.4}, upper {:.4}, score_min {:.4}, cyc_min {:.4}'.format(i,
                ...
                if self.dag:
                    ...
                    self._log.info('before pruning: fdr {}, tpr {}, fpr {}, shd {}, nnz {}'.format(fdr, tpr, fpr, shd, nnz))
                    self._log.info('after  pruning: fdr {}, tpr {}, fpr {}, shd {}, nnz {}'.format(fdr2, tpr2, fpr2, shd2, nnz2))
