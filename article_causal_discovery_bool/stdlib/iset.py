...
def _ibooltable(k: int, n: int, c=True) -> list[int]:
    """
...
def truth_table(bf: list[int], flatten=True) \
        -> Union[list[list[int]], list[tuple[list[int], int]]]:
    M = len(bf)
    n = ilog2(M)
    tt = []
...
def ibooltable(k: int, n: int, allvars=False, const=True, vars=None):
    if not allvars or k >= 0:
        return _ibooltable(k, n, const)
    ...
    # allvars=True) use all variables
    #  const=False) exclude constant functions

    bfun = _ibooltable(k, n)
    tt = truth_table(bfun)
    expr = simplify_expression(vars, tt)
    while (allvars and not use_all_vars(vars, expr)) or (not const and expr in ["0", "1"]):
        bfun = _ibooltable(k, n)
        tt = truth_table(bfun)
        expr = simplify_expression(vars, tt)
    return bfun, expr
