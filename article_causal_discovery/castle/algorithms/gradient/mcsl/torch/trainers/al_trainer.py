...
class Trainer(object):
    ...
    def __init__(self, model, learning_rate, init_rho, rho_thresh, h_thresh,
        ...
        self.logging = logging.getLogger('castle.algorithm.trainer')
    ...
    def train(self, x, max_iter, iter_step) -> torch.Tensor:
        ...
        for i in range(1, max_iter + 1):
            self.logging.info(f'Current epoch: {i}==================')
            ...
            self.logging.info(f'Current        h: {h_new}')
            self.logging.info(f'Current h_logits: {h_logits}')
            if (h_new <= self.h_tol
    ...
    def train_step(self, x, iter_step, rho, alpha, temperature) -> tuple:
        ...
        for _ in range(iter_step):
            ...
            if _ % LOG_FREQUENCY == 0:
                self.logging.info(f'Current loss in step {_}: {curr_loss.detach()}')
