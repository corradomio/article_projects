...
class NotearsNonlinear(BaseLearner):
    ...
    def __init__(self, lambda1: float = 0.01,
        ...
        self.logging = logging.getLogger('castle.algorithm.notears_nonlinear')
    ...
    def notears_nonlinear(self,
        ...
        """
        self.logging.info('[start]: n={}, d={}, iter_={}, h_={}, rho_={}'.format(
            X.shape[0], X.shape[1], self.max_iter, self.h_tol, self.rho_max))

        for _ in range(self.max_iter):
            self.dual_ascent_step(model, X)

            self.logging.debug('[iter {}] h={:.3e}, rho={:.1e}'.format(_, self.h, self.rho))
    ...
    def get_model(self, input_dim):
        ...
        if self.model_type == "mlp":
            ...
            self.logging.info(f'Unsupported model type {self.model_type}.')
