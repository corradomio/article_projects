...
def compare_models(g, Xtr, ytr, Xte, yte):
    ...
    use_model(g, "linear-1", linear, Xtr_scaled, ytr_scaled, Xte_scaled, yte_scaled, fh)

    linear = sktfl.LinearForecaster(
        estimator=LinearRegression,
        lags=24, tlags=3
    )
    use_model(g, "linear-3", linear, Xtr_scaled, ytr_scaled, Xte_scaled, yte_scaled, fh)

    linear = sktfl.LinearForecaster(
        estimator=LinearRegression,
        lags=24, tlags=6
    )
    use_model(g, "linear-6", linear, Xtr_scaled, ytr_scaled, Xte_scaled, yte_scaled, fh)

    linear = sktfl.LinearForecaster(
        estimator=LinearRegression,
        lags=24, tlags=12
    )
    use_model(g, "linear-12", linear, Xtr_scaled, ytr_scaled, Xte_scaled, yte_scaled, fh)

    linear = sktfl.LinearForecaster(
        estimator=LinearRegression,
        lags=24, tlags=12, flatten=True
    )
    use_model(g, "linear-12-flatten", linear, Xtr_scaled, ytr_scaled, Xte_scaled, yte_scaled, fh)
    ...
    use_model(g, "knn-1", linear, Xtr_scaled, ytr_scaled, Xte_scaled, yte_scaled, fh)

    linear = sktfl.LinearForecaster(
        estimator=KNeighborsRegressor,
        lags=24, tlags=3
    )
    use_model(g, "knn-3", linear, Xtr_scaled, ytr_scaled, Xte_scaled, yte_scaled, fh)

    linear = sktfl.LinearForecaster(
        estimator=KNeighborsRegressor,
        lags=24, tlags=6
    )
    use_model(g, "knn-6", linear, Xtr_scaled, ytr_scaled, Xte_scaled, yte_scaled, fh)

    linear = sktfl.LinearForecaster(
        estimator=KNeighborsRegressor,
        lags=24, tlags=12
    )
    use_model(g, "knn-12", linear, Xtr_scaled, ytr_scaled, Xte_scaled, yte_scaled, fh)

    linear = sktfl.LinearForecaster(
        estimator=KNeighborsRegressor,
        lags=24, tlags=12, flatten=True
    )
    use_model(g, "knn-12-flatten", linear, Xtr_scaled, ytr_scaled, Xte_scaled, yte_scaled, fh)
